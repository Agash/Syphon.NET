...
public static class IOSurfaceExtensions
{
    ...
    public static bool IsBgra(this IOSurface.IOSurface surface) =>
        surface.PixelFormat == FourCcBgra;
    ...
    /// </remarks>
    public static int PlaneCount(this IOSurface.IOSurface surface) =>
        Math.Max(1, (int)surface.PlaneCount);
    ...
    public static (int Width, int Height, int BytesPerRow) PlaneInfo(
        this IOSurface.IOSurface surface,
        int plane
    )
    {
        ...
        return (
            (int)surface.GetWidth((nuint)plane),
            (int)surface.GetHeight((nuint)plane),
            (int)surface.GetBytesPerRow((nuint)plane)
        );
    }
    ...
    /// </summary>
    public static LockedSurface LockBytes(this IOSurface.IOSurface surface, bool readOnly) =>
        new(surface, readOnly);
    ...
    public static int CopyTightlyPacked(
        this IOSurface.IOSurface surface,
    ...
    )
    {
        ...
        if (destination.Length < needed)
            throw new ArgumentException(
                $"Destination too small: need {needed} bytes.",
                nameof(destination)
            );
    ...
    )
    {
        ...
        if (source.Length < rowBytes * height)
            throw new ArgumentException(
                "Source is smaller than Width * bytesPerPixel * Height.",
                nameof(source)
            );
    ...
    public readonly ref struct LockedSurface
    {
        ...
        public unsafe Span<byte> Bytes =>
            new((void*)_surface.BaseAddress, (int)_surface.AllocationSize);
    ...
    public static IOSurface.IOSurface Create(
        int width,
        int height,
        CVPixelFormatType format,
    ...
    public static IOSurface.IOSurface CreateBgra(int width, int height) =>
        Create(width, height, CVPixelFormatType.CV32BGRA);
}
